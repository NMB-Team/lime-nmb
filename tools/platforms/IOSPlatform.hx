...
class IOSPlatform extends PlatformTarget {
			...
			if (!StringTools.endsWith(dependency.name, ".framework")
				...
				if (dependency.path != "") {
					...
					if (dependency.forceLoad) {
						project.config.push("ios.linker-flags", "-force_load $SRCROOT/$PRODUCT_NAME/lib/$CURRENT_ARCH/"
	...
	override public function rebuild():Void {
		...
		if (armv6)
			commands.push(["-Dios", "-DHXCPP_CPP11", "-DHXCPP_ARMV6"]);
		if (armv7)
			commands.push(["-Dios", "-DHXCPP_CPP11", "-DHXCPP_ARMV7"]);
		if (armv7s)
			commands.push(["-Dios", "-DHXCPP_CPP11", "-DHXCPP_ARMV7S"]);
		if (arm64)
			commands.push(["-Dios", "-DHXCPP_CPP11", "-DHXCPP_ARM64"]);
		if (i386)
			commands.push(["-Dios", "-Dsimulator", "-DHXCPP_M32", "-DHXCPP_CPP11"]);
		if (x86_64)
			commands.push(["-Dios", "-Dsimulator", "-DHXCPP_M64", "-DHXCPP_CPP11"]);
	...
	override public function update():Void {
		...
		System.copyFileTemplate(project.templatePaths, "iphone/PROJ/PROJ-Info.plist", projectDirectory
			+ "/"
			+ project.app.file
			+ "-Info.plist", context, true, false);
		System.copyFileTemplate(project.templatePaths, "iphone/PROJ/PROJ-Prefix.pch", projectDirectory
			+ "/"
			+ project.app.file
			+ "-Prefix.pch", context, true, false);
