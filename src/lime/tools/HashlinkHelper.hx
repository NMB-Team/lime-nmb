	...
	public static function copyHashlink(project:HXProject, targetDirectory:String, applicationDirectory:String, executablePath:String, ?is64 = true) {
		...
		if (hlPath == null) {
			...
			if (project.targetFlags.exists("hlc")) {
				...
				var includeDirectory = sys.FileSystem.exists(Path.combine(limeDirectory, "project")) ? Path.combine(limeDirectory, "project/lib/hashlink/src") : Path.combine(limeDirectory, "templates/bin/hl/include");
